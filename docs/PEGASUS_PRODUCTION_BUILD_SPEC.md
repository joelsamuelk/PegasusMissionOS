...
| 5 | Published reports don't silently change | 🟢 Upheld in the application layer, unpersisted | **MG-5.** Versions are immutable and pin a snapshot carrying each figure's *rendered value* as well as its claim id, so a superseded claim surfaces as computable drift rather than altering the published document. The half that was never enforced — **a number typed into prose is not a citation** — now is: `detectUncitedFigures` blocks approval on an uncited money or percentage figure. Persistence still waits on MG-2, so the invariant holds against the in-memory adapter only. | ✅ **MG-5**; storage **MG-2** |
