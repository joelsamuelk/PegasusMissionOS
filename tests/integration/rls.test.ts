 ...
 *
 * This file creates auth users and organisations in whatever project the
 * environment points at, so it takes an explicit opt-in as well -- the same
 * one the contract suite takes. A developer who happens to have these exported
 * in their shell should not discover that `npm test` wrote to their live
 * project. `npm run test:integration` sets it.
 */
const configured =
  process.env.PEGASUS_CONTRACT_DB === "1" && Boolean(URL && ANON && SERVICE);
